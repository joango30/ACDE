...
type RestOf<T extends ITraining | NewTraining> = Omit<T, 'startDate' | 'endDate'> & {
...
export class TrainingService {
  ...
  protected convertDateFromClient<T extends ITraining | NewTraining | PartialUpdateTraining>(training: T): RestOf<T> {
    return {
      ...training,
      startDate: training.startDate?.format(DATE_FORMAT) ?? null,
      endDate: training.endDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTraining: RestTraining): ITraining {
    return {
      ...restTraining,
      startDate: restTraining.startDate ? dayjs(restTraining.startDate) : undefined,
      endDate: restTraining.endDate ? dayjs(restTraining.endDate) : undefined,
    };
  }
